...
## Быстрый запуск
Готовый `.exe` можно найти в папке [builds](./builds/) (рекомендую последнюю версию). Там лежат все сборки со инструкциями по использованию.
...
лежать в одной папке со скриптом, со след. полями (* - обязательное):
    - *bot_token - (str) токен бота (в формате 0000000:aaaaaaa...)
    - *chat_id - (str) id чата в телеграмме (может быть отрицательным)
    - max_time - (int) максимальное время работы скрипта в минутах (по дефолту 60)
    - push_message_btn - (int) код клавишы отправки скрина (по дефолту КОЛСИКО МЫШИ (4))
    - exit_btn_code - (int) - код клавишы выхода (по дефолту TAB (9)) [примеры клавиш](https://stackoverflow.com/questions/31363860/how-do-i-get-the-name-of-a-key-in-pywin32-giving-its-keycode) 
    - printer - (str) вариант уведомлений (console, telegram, window) (по дефолту 60)
