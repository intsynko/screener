...
1. Откройте [генератор config.json](https://intsynko.github.io/screener/pages/config-generator/) — выберите версию скринера, заполните форму и скачайте готовый `config.json` и `main.exe`.
2. Положите оба файла в одну папку и запустите `main.exe`.

> **Старый способ:** готовые сборки можно вручную искать в папке [builds](./builds/) (рекомендуем последнюю версию). Там лежат все версии с инструкциями по использованию.
...
- На странице сообщества в разделе «Управление» → «Дополнительно» → «Работа с API» → «Создать ключ» получить
токен с правом на сообщения и фотографии
- На странице сообщества в разделе «Управление → «Чаты» → «Создать чат»
